...
class Delegate {
  ...
  public clear(): void {
    ...
    if (this._flag != 0) {
      arr.forEach((value, index, arr) => {
        if (index % ITEM_LAYOUT == 3) arr[index] = 0
      })
      this._flag = 2
    } else {
      arr.length = 0
  ...
  public clearForTarget(target: any): void {
    ...
    if (this._flag != 0) {
      arr.forEach((value, index, arr) => {
        if (index % ITEM_LAYOUT == 1 && arr[index] == target) arr[index + 2] = 0
      })
      this._flag = 2
    } else {
  ...
  public invoke(...args: any[]): void {
    ...
    for (let i = 0; i < cnt; i += ITEM_LAYOUT) {
      ...
      if (fixedArgs != null) arr[i].call(arr[i + 1], ...fixedArgs, ...args)
      else arr[i].call(arr[i + 1], ...args)
      if (arr[i + 3] == 2) {
        arr[i + 3] = 0
        this._flag = 2
      }
