...
class EventDispatcher {
  /** @private */
  private _events: Map<string, Delegate> = new Map<string, Delegate>()
  ...
  event(type: string, data?: any): boolean {
    ...
    if (Array.isArray(data)) {
      ...
      const ev = eventPool.length > 0 ? eventPool.pop()! : new Event()
  ...
  offAllCaller(caller: any): EventDispatcher {
    if (caller && this._events) {
      for (const type in this._events) {
        const listeners = this._events.get(type)!
        listeners.clearForTarget(caller)
      }
    }
    return this
  }
