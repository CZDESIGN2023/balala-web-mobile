  ...
  methods: {
    ...
    touchScale(e) {
      ...
      const oldL = Math.sqrt(
        (oldTouch1.x - oldTouch2.x) ** 2
        + (oldTouch1.y - oldTouch2.y) ** 2,
      )
      const newL = Math.sqrt(
        (newTouch1.x - newTouch2.x) ** 2
        + (newTouch1.y - newTouch2.y) ** 2,
      )
      const cha = newL - oldL
    ...
    checkCropLimitSize() {
      ...
      // 限制最小宽度和高度
      cropW = Number.parseFloat(limitMinNum[0])
      cropH = Number.parseFloat(limitMinNum[1])
      return [cropW, cropH]
    },
    // 结束改变
    changeCropEnd(e) {
    ...
    reload() {
      ...
      img.onload = () => {
        ...
        // 读取图片的旋转信息
        // 得到外层容器的宽度高度
        this.w = Number.parseFloat(window.getComputedStyle(this.$refs.cropper).width)
        this.h = Number.parseFloat(window.getComputedStyle(this.$refs.cropper).height)
      ...
      switch (arr[0]) {
        ...
        default:
          try {
            ...
            if (str.search('px') !== -1) {
              str = str.replace('px', '')
              imgW = Number.parseFloat(str)
              const scaleX = imgW / this.trueWidth
              let scaleY = 1
              let strH = arr[1]
              if (strH.search('px') !== -1) {
                strH = strH.replace('px', '')
                imgH = Number.parseFloat(strH)
            ...
            if (str.search('%') !== -1) {
              str = str.replace('%', '')
              imgW = (Number.parseFloat(str) / 100) * this.w
              scale = imgW / this.trueWidth
            }
            ...
            if (arr.length === 2 && str === 'auto') {
              ...
              if (str2.search('px') !== -1) {
                ...
                imgH = Number.parseFloat(str2)
                scale = imgH / this.trueHeight
              }
              if (str2.search('%') !== -1) {
                str2 = str2.replace('%', '')
                imgH = (Number.parseFloat(str2) / 100) * this.h
                scale = imgH / this.trueHeight
              }
            }
          }
    ...
    goAutoCrop(cw, ch) {
      ...
      // 截图框默认大小
      // 如果为0 那么计算容器大小 默认为80%
      let w = cw || Number.parseFloat(this.autoCropWidth)
      let h = ch || Number.parseFloat(this.autoCropHeight)
      if (w === 0 || h === 0) {
        w = maxWidth * 0.8
        h = maxHeight * 0.8
      }
