...
import { getWorkItemRoleList, workItemRoleCreate, workItemRoleNameSet } from '@/api/workflow'
...
// 获取状态列表
function queryRoleOptions() {
  return new Promise((resolve, reject) => {
    getWorkItemRoleList({ spaceId: spaceId || props.spaceId })
      .then((d) => {
        resolve(d.data.list
...
function onAdd(label: string) {
  return new Promise((res, rej) => {
    ...
    workItemRoleCreate({ spaceId: spaceId || props.spaceId, name: newLabel })
      .then(() => {
        message.success({ content: '角色创建成功' })
        res(true)
      })
      .catch((e) => {
...
function onEdit(item: any) {
  return new Promise((res, rej) => {
    ...
    workItemRoleNameSet({ spaceId: spaceId || props.spaceId, name: label, id: item.id })
      .then(() => {
        message.success({ content: '角色名称编辑成功' })
        return res(true)
      })
      .catch((e) => {
...
<template>
  <BSelectSyncEdit
    ...
    v-bind="$attrs"
    :query-options="queryRoleOptions"
    :add-fn="onAdd"
    :edit-fn="onEdit"
    placeholder="请选择节点关联角色"
    search-placeholder="搜索角色"
    edit-placeholder="输入角色名称，回车确认"
    create-text="添加角色"
  />
</template>
